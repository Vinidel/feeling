...
`deno task start` allowlists that disposable local Postgres on `127.0.0.1:55322`
and `localhost:55322` in addition to the Auth0 JWKS host and the Steady Sydney
Supavisor transaction endpoint. The container image stays limited to Auth0 and
Supavisor. Changing either hosted host requires a reviewed image-permission
update rather than silently expanding network access.
