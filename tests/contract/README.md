...
`run.mjs` is a base-URL-driven smoke layer for comparing the existing Go service
with later replacement slices. It never contains credentials or private
fixtures.
...
Stage 10 captures weekly source and target observations from Gin/Mongo-mock and
Deno/disposable-Postgres combinations. `compare-weekly.mjs` rejects every
difference except the approved database-timestamp and strict-mood-validation
normalizations. Together with the Stage 8 feelings comparator, it accounts for
all supported replacement API behavior before the retired surfaces are omitted.
...
Authenticated read-shape checks additionally require an isolated test account
...
Do not point this runner at production without separate authorization. Do not
commit or log the environment values. The deterministic handler-level source
characterization lives in `server/api_characterization_test.go` and uses MongoDB
wire-protocol mocks plus locally signed test JWTs.
