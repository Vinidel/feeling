...
contractTest('supported routes reject a missing bearer token', async () => {
  for (const path of ['/api/feelings', '/api/weekly-tracker?weekOf=2026-01-05']) {
...
contractTest('authenticated reads preserve their top-level source shapes', { skip: !accessToken || !userId }, async () => {
  const headers = {
  ...
  const weekly = await request('/api/weekly-tracker?weekOf=2026-01-05', { headers });
