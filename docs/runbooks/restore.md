...
1. Verify the separate `Steady backups` project is reachable. If Supabase has
   auto-paused the Free project, resume that exact project, wait for
   `ACTIVE_HEALTHY`, and confirm the private bucket and retained inventory
   before selecting an object. Do not upgrade an account merely to perform the
   operator restore.
2. Select the exact qualified encrypted backup by recorded object path,
   checksum, manifest counts, and successful prior restore result. Never select
   an object explicitly recorded as unqualified.
3. Download it into a new owner-only scratch directory and verify the
   ciphertext checksum before decryption.
4. Read the encryption key from the secret store without displaying it and run
   ...
   stop the restore.
5. Confirm the manifest reports the selected environment, Sydney source and
...
6. Create a new empty disposable PostgreSQL 17 database. Restore roles, schema,
   then data; do not restore password values.
7. Compare the manifest migration list with the source and version-controlled
   ...
   checks.
8. For a retained historical Mongo checkpoint, import into a new disposable
   database and compare
   collection counts and keyed migration reconciliation reports.
9. Record elapsed time and all sanitized results. A successful command is not
   enough: the restored application must pass API and authenticated read smoke
   tests.
10. Remove only the exact disposable database and plaintext scratch directory
